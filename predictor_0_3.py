...
####### M A I N #####################
with open('trace_sample.txt', "r") as f:
    code = f.readlines()
...
for i in range(0, len(code)-2):
    ...
    if (pc_plus1 - pc == 4):
        ...
        # Update to deal with the condition (hex_pc[4:6]== 'fc' and hex_pc_plus1[4:6] == '00')
        if in_BTB(entry, hex_pc) == True:
            keep = btb[entry % 1024][1]
            pred = update_pred(prev_pred=btb[entry][2], t_nt=NOT_TAKEN)
            update_BTB(entry =entry, pc=hex_pc, tpc=keep, local_pred=pred)
        else:
            pass
...
outputbtb1 = print_BTB(sort=True)
